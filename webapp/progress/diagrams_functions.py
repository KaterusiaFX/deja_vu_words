...
def frenchword_dataframe(username):
    username = User.query.filter_by(username=username).first_or_404()
    userdict = process_user_frenchdict_index(username)
    if userdict:
        first_date = userdict[-1][2]
        today = datetime.now()
        delta = today - first_date
...
def new_frenchwords_accumulated(username):
    sns.set(style="ticks", palette="colorblind", font="Arial")
    df_dict = frenchword_dataframe(username)
    if df_dict:
        df = pd.DataFrame(df_dict)
        new_frenchwords_accumulation = sns.relplot(
            x="дни",
            y="добавленные слова",
            kind="line",
            data=df,
        ...
        new_frenchwords_accumulation.fig.autofmt_xdate()
        new_frenchwords_accumulation.set_xticklabels(rotation=90, horizontalalignment='center')
        plt.savefig(f'webapp/static/progress/new_frenchwords_{username}.png')
        return f'progress/new_frenchwords_{username}.png'
    return None


def familiar_frenchwords_accumulated(username):
    sns.set(style="ticks", palette="colorblind", font="Arial")
    df_dict = frenchword_dataframe(username)
    if df_dict:
        if sum(df_dict['изученные слова']):
            df = pd.DataFrame(df_dict)
            familiar_frenchwords_accumulation = sns.relplot(
                x="дни",
                y="изученные слова",
                kind="line",
                data=df,
            ...
            familiar_frenchwords_accumulation.fig.autofmt_xdate()
            familiar_frenchwords_accumulation.set_xticklabels(rotation=90, horizontalalignment='center')
            plt.savefig(f'webapp/static/progress/familiar_frenchwords_{username}.png')
            return f'progress/familiar_frenchwords_{username}.png'
        return None
    return None


def difficult_frenchwords_catplot(username):
    username = User.query.filter_by(username=username).first_or_404()
    userdict = process_user_frenchdict_index(username)
    failure_dict = {'слова': [], 'неудачные попытки': []}
    if userdict:
        ...
        difficult_frenchwords.fig.autofmt_xdate()
        difficult_frenchwords.set_xticklabels(rotation=90, horizontalalignment='center')
        plt.savefig(f'webapp/static/progress/difficult_frenchwords_{username.username}.png')
        return f'progress/difficult_frenchwords_{username.username}.png'
    return None
