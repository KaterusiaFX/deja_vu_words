...
def engword_translation_training(username, guess_word, user_answer):
    ...
    if guess_word[0].translation_rus == user_answer:
        word.word_translation = None  # if the word is learned, then NULL is written to the cel
        db.session.commit()
    ...
    word.word_translation += 1  # if the user made a mistake with the translation, then increase the counter of attempts by 1
    db.session.commit()
